...
int main(int argc, const char * argv[]) {
	...
	/* detector parameters */
	float det_ang_min, det_ang_max;
    float fiber_dia[100]; 
    float fiber_loc_x[100], fiber_loc_y[100]; 
    int NoFiber; 
	...
	fgets(buf, 32, fid);
		...
		fgets(buf,32,fid);
		sscanf(buf,"%f", &det_ang_min);
		fgets(buf,32,fid);
		sscanf(buf,"%f", &det_ang_max);
		fgets(buf,32,fid);
		sscanf(buf,"%d",&NoFiber);
		for (i=0; i<NoFiber; i++){
				fgets(buf, 32, fid);
				sscanf(buf, "%f", &fiber_loc_x[i]);
				fgets(buf, 32, fid);
				sscanf(buf, "%f", &fiber_loc_y[i]);
				fgets(buf, 32, fid);
				sscanf(buf, "%f", &fiber_dia[i]);
		}
...
/*	strcpy(filename,myname);
	...
	double	*RX_real=NULL;
	double	*RX_imag=NULL;
	double	*RXa_real=NULL;
	double	*RXa_imag=NULL;
	...
	RXa_real  = (double *)malloc(Nxy*sizeof(double));
	RXa_imag  = (double *)malloc(Nxy*sizeof(double));
	...
	double	*RM_real=NULL;
	double	*RM_imag=NULL;
	double	*RMa_real=NULL;
	double	*RMa_imag=NULL;
	//double 	*Forigin=NULL;
	...
	RMa_real  = (double *)malloc(Nxy*sizeof(double));
	RMa_imag  = (double *)malloc(Nxy*sizeof(double));	
    ...
    mcxyz(Nphotons, dx, dy, dz, Nx, Ny, Nz, xs, ys, zs, ux0, uy0, uz0, mod_freq, mcflag, launchflag, boundaryflag, pfuncflag, outflag, printprogress, n_inv, n_outv, xfocus, yfocus, zfocus,
	    	 radius, waist, muav, musv, gv, gammav,mua_multv, mus_multv, muaxfv, muaf_multv, Qeffv, tauv, FX_real, FX_imag, RX_real, RX_imag, RXa_real, RXa_imag, FM_real, FM_imag, 
			 RM_real, RM_imag, RMa_real, RMa_imag, vX, vF, xface, det_ang_min, det_ang_max, fiber_dia, fiber_loc_x, fiber_loc_y, NoFiber);
 ...
 //   getchar();
    ...
    for (i=0; i<NN;i++){
      ...
      FX_real[i] = (btemp*omega_by_c + atemp*muav[vX[i]])/(omega_by_c*omega_by_c + muav[vX[i]]*muav[vX[i]])/temp;
      FX_imag[i] = -(atemp*omega_by_c - btemp*muav[vX[i]])/(omega_by_c*omega_by_c + muav[vX[i]]*muav[vX[i]])/temp;
    ...
    strcpy(filename,myname);
    strcat(filename,"_RXareal.bin");
    printf("saving %s\n",filename);
    fid = fopen(filename, "wb");   /* 2D voxel output */
    fwrite(RXa_real, sizeof(double), Nxy, fid);
    fclose(fid);
    
    strcpy(filename,myname);
    strcat(filename,"_RXaimag.bin");
    printf("saving %s\n",filename);
    fid = fopen(filename, "wb");   /* 2D voxel output */
    fwrite(RXa_imag, sizeof(double), Nxy, fid);
    fclose(fid);
...
//	int Nyx = Ny*Nx;
    ...
    strcpy(filename,myname);
    strcat(filename,"_RMareal.bin");
    printf("saving %s\n",filename);
    fid = fopen(filename, "wb");   /* 2D voxel output */
//	int Nyx = Ny*Nx;
    fwrite(RMa_real, sizeof(double), Nxy, fid);
    fclose(fid);
    
    strcpy(filename,myname);
    strcat(filename,"_RMaimag.bin");
    printf("saving %s\n",filename);
    fid = fopen(filename, "wb");   /* 2D voxel output */
    fwrite(RMa_imag, sizeof(double), Nxy, fid);
    fclose(fid);
