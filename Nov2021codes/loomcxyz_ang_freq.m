...
j=j+1;
outflag = A(j); j=j+1;
det_ang_min = A(j); j=j+1;
det_ang_max = A(j); j = j+1; 
NoFiber = A(j); j = j+1;
for ii=1:NoFiber
    DetLoc(ii,1) = A(j); j = j+1; 
    DetLoc(ii,2) = A(j); j = j+1; 
    DetFiber_dia(ii) = A(j); j  =j+1;
end
...
% Load diffuse fluence
filename = sprintf('%s_RXareal.bin',myname);
disp(['loading ' filename])
tic
fid = fopen(filename, 'rb');
[Data count] = fread(fid, NoFiber, 'double');
fclose(fid);
toc
RX_ang_real = Data ; % reshape(Data,Ny,Nx); % F(y,x,z)

filename = sprintf('%s_RXaimag.bin',myname);
disp(['loading ' filename])
tic
fid = fopen(filename, 'rb');
[Data count] = fread(fid, NoFiber, 'double');
fclose(fid);
toc
RX_ang_imag = Data; %reshape(Data,Ny,Nx); % F(y,x,z)
...
fclose(fid);
toc
RX_real = Data ; %reshape(Data,Nx,Ny); % F(y,x,z)
...
fclose(fid);
toc
RX_imag =  Data; %reshape(Data,Nx,Ny); % F(y,x,z)
...
% 
% figure(150)
RX_ang = RX_ang_real + 1i*RX_ang_imag;
% subplot(1,2,1)
% imagesc(x(ux),y(uy),real(log10(RX_ang(ux,uy))))
% colorbar
% xlabel('x [cm]')
% ylabel('y [cm]')
% title('Magnitude - RX')
% 
% subplot(1,2,2)
% imagesc(x(ux),y(uy),imag(log10(RX_ang(ux,uy))))
% colorbar
% xlabel('x [cm]')
% ylabel('y [cm]')
% title('Phase - RX')
...
subplot(211)
plot(1:NoFiber, real(log10(RX))); 
hold on
plot(1:NoFiber, real(log10((RX_ang))));
xlabel('Fiber Number')
ylabel('log(j^{+x})')
subplot(212)
plot(1:NoFiber, imag(log10(RX))*180/pi); 
hold on
plot(1:NoFiber, imag(log10(RX_ang))*180/pi); 
xlabel('Fiber Number')
ylabel('\angle(j^{+x})')
...
if (emflag)
    ...
    filename = sprintf('%s_RMareal.bin',myname);
    disp(['loading ' filename])
    tic
    fid = fopen(filename, 'rb');
    [Data count] = fread(fid, NoFiber, 'double');
    fclose(fid);
    toc
    RM_ang_real = Data; %reshape(Data,Ny,Nx); % F(y,x,z)
    
    filename = sprintf('%s_RMaimag.bin',myname);
    disp(['loading ' filename])
    tic
    fid = fopen(filename, 'rb');
    [Data count] = fread(fid, NoFiber, 'double');
    fclose(fid);
    toc
    RM_ang_imag = Data; %reshape(Data,Ny,Nx); % F(y,x,z)
    ...
    fclose(fid);
    toc
    RM_real = Data; %reshape(Data,Ny,Nx); % F(y,x,z)
    ...
    fclose(fid);
    toc
    RM_imag = Data; %reshape(Data,Ny,Nx); % F(y,x,z)
...
%     
%       figure (251)
    RM_ang = RM_ang_real + 1i*RM_ang_imag;
%     subplot(1,2,1)
%     imagesc(x(ux),y(uy),real(log10(RM_ang(ux,uy))));
%     xlabel('x [cm]')
%     ylabel('y [cm]')
%     title('Magnitude Rm')
%     
%     subplot(1,2,2)
%     imagesc(x(ux),y(uy),imag(log10(RM_ang(ux,uy))))
%     xlabel('x [cm]')
%     ylabel('y [cm]')
%     title('Phase Rm')
%         
...
subplot(211)
plot(1:NoFiber, real(log10(RM))); 
hold on
plot(1:NoFiber, real(log10((RM_ang))));
xlabel('Fiber Number')
ylabel('log(j^{+m})')
subplot(212)
plot(1:NoFiber, imag(log10(RM))*180/pi); 
hold on
plot(1:NoFiber, imag(log10(RM_ang))*180/pi); 
xlabel('Fiber Number')
ylabel('\angle(j^{+m})')
