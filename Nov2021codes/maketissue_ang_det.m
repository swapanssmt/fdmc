...
myname      = 'Test_S45_1';% name for files: myname_T.bin, myname_H.mci
...
mod_freq = 0; % modulation frequency in [MHZ]
...
radius      = 0.0025; %0.00400;   % 1/e radius of beam at tissue surface
...
if launchflag~=0
    ...
    refr_source =1.5; refr_medium = 1.37;
...
xc1 = 0; zc1 = 0.02; yc1 = 0;
% xc2 = 0.1; zc2 = 0.25; yc2 = 0;
...
%                         yd = y(iy) - yc2;
%                         r  = sqrt(xd^2 + zd^2 + yd^2);	% r from inhomogeneity centre
%                         if (r<=r2)     	% if r is within inhomogeneity
%                             TF(iy,ix,iz) = 4; % fluorophore
%                         end
%             
%                         xd = x(ix) - xc3;
...
DetectorLoc = [XX(:),YY(:),0*ones(length(YY(:)),1)];

NofDetectors = size(DetectorLoc, 1);
Detfiber_dia = 0.0025*ones(NofDetectors,1);
det_ang_min =  40;
det_ang_max =  50;
