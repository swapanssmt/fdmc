...
int get_fiber_number(double x,double y, float *fiber_loc_x, float *fiber_loc_y, float *fiber_dia, int NoFiber);
...
void mcxyz(double Nphotons, float dx, float dy, float dz, int Nx, int Ny, int Nz, float xs, float ys, float zs, float ux0, float uy0, float uz0, float mod_freq,
			...
			double *RX_real, double *RX_imag, double *RX_ang_real, double *RX_ang_imag, double *FM_real, double *FM_imag, double *RM_real, double *RM_imag, double *RM_ang_real, double *RM_ang_imag,
			char *vX, char *vF, char *xface, float det_ang_min, float det_ang_max, float *fiber_dia, float *fiber_loc_x, float*fiber_loc_y, int NoFiber ){
...
//	printf("%s\n", ctime(&now));	
	...
	for(j=0; j<Nxy;j++){ RX_real[j] = 0;	RX_imag[j]=0;
					RM_real[j] = 0;	RM_imag[j]=0;
					RX_ang_real[j]=0; RX_ang_imag[j]=0;
					RM_ang_real[j]=0; RM_ang_imag[j]=0;
					} // ensure F[] starts empty.	
			...
			do{  // while sleft>0
				...
				else /* photon has crossed voxel boundary */
				{
                    ...
                    if (boundaryflag==0) { // Infinite medium.
						...
						if (iz<0)   {iz=0; 	
					//	ri = (long) (ix*Ny +iy);
						ri = get_fiber_number(x,y,fiber_loc_x, fiber_loc_y, fiber_dia, NoFiber);
						xtype = xface[(long) ix*Ny + iy];
						rnd = RandomGen(1,0,NULL);
						...
						if(rnd>=Ref)
						{
							...
							if (outflag==1){
								...
								//	 Forigin[fo_index]+=1;
									...
									if ((acos(uz1)<=det_ang_max)&&(acos(uz1)>=det_ang_min)){
										if (emflag==1)	{
											RM_ang_real[ri]+= W*cos(Wphase)*uz1;
										 	RM_ang_imag[ri]-= W*sin(Wphase)*uz1;
										 
										}
										else {
											RX_ang_real[ri]+= W*cos(Wphase)*uz1;
										 	RX_ang_imag[ri]-= W*sin(Wphase)*uz1;
										}
									}
							}
							...
							if (emflag==1){
								...
								//	 Forigin[fo_index]+=1;
									...
									if ((acos(uz1)<=det_ang_max)&&(acos(uz1)>=det_ang_min)){
										if (emflag==1)	{
											RM_ang_real[ri]+= W*cos(Wphase);
										 	RM_ang_imag[ri]-= W*sin(Wphase);
										}
										else {
											RX_ang_real[ri]+= W*cos(Wphase);
										 	RX_ang_imag[ri]-= W*sin(Wphase);
										}
									}
...
int get_fiber_number(double x,double y,float *fiber_loc_x, float *fiber_loc_y, float *fiber_dia, int NoFiber)
{
	...
	double r_in;
	while(ii<NoFiber){
		r_in = (x-fiber_loc_x[ii])*(x-fiber_loc_x[ii])+ (y - fiber_loc_y[ii])*(y - fiber_loc_y[ii]) - fiber_dia[ii]*fiber_dia[ii];
		if (r_in<0){
			return(ii);
			break;
		}
